...
class LS3DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'image': torch.tensor(sample['image']),
                ...
                'bbox': self.get_formatted_bbox(sample['bbox'], sample['image'].shape[1:])}
    ...
    def apply_albu_transform(self, sample):
        ...
        bbox = self.get_formatted_bbox(torch.tensor(transformed_sample['bboxes']),
                                       transformed_sample['image'].shape[1:])
    ...
    def draw_idx(self, idx):
        ...
        ax.imshow(sample['image'].permute(2, 1, 0))
        points = sample['landmarks_2d'][0]
    ...
    def draw_sample(self, sample):
        fig, ax = plt.subplots(1)
        ax.imshow(sample['image'][0].permute(2, 1, 0))
        points = sample['landmarks_2d'][0]
        ax.scatter(points[:, 0], points[:, 1])
        bbox = self.get_bbox(points)
        rect = patches.Rectangle((bbox[0], bbox[1]), bbox[2], bbox[3], linewidth=1, edgecolor='r', facecolor='none')
